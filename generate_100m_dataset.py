...
def generate_100m_dataset(output_dir=None):
    if output_dir is None:
        ...
        parent_sample_dir = os.path.join(os.path.dirname(base_dir), "ornek_veri_setleri")
        output_dir = parent_sample_dir if os.path.exists(os.path.dirname(base_dir)) else base_dir
    ...
    sehir_bolge_pairs = [
        ...
        ("Konya", "İç Anadolu"), ("Gaziantep", "Güneydoğu Anadolu"), ("Kocaeli", "Marmara"),
        ("Mersin", "Akdeniz"), ("Kayseri", "İç Anadolu"), ("Eskişehir", "İç Anadolu"),
        ("Diyarbakır", "Güneydoğu Anadolu"), ("Samsun", "Karadeniz"), ("Denizli", "Ege"),
        ("Şanlıurfa", "Güneydoğu Anadolu"), ("Sakarya", "Marmara"), ("Trabzon", "Karadeniz"),
        ("Malatya", "Doğu Anadolu"), ("Erzurum", "Doğu Anadolu"), ("Manisa", "Ege"),
        ("Balıkesir", "Marmara"), ("Kahramanmaraş", "Akdeniz"), ("Van", "Doğu Anadolu"),
        ("Aydın", "Ege"), ("Tekirdağ", "Marmara"), ("Muğla", "Ege"),
        ("Hatay", "Akdeniz"), ("Ordu", "Karadeniz"), ("Sivas", "İç Anadolu")
    ]
    ...
    sehir_weights = np.array([
        0.18, 0.09, 0.07, 0.05, 0.045, 0.035, 0.035, 0.035, 0.035, 0.03,
        0.025, 0.025, 0.025, 0.025, 0.025, 0.02, 0.02, 0.02, 0.02, 0.018,
        0.018, 0.018, 0.016, 0.015, 0.015, 0.015, 0.015, 0.015, 0.012, 0.013
    ])
    sehir_weights /= sehir_weights.sum()

    kat_alt_pairs = [
        ("Elektronik", "Akıllı Telefon"), ("Elektronik", "Dizüstü Bilgisayar"), ("Elektronik", "Tablet & Aksesuar"),
        ("Ev & Yaşam", "Mobilya"), ("Ev & Yaşam", "Küçük Ev Aletleri"), ("Ev & Yaşam", "Aydınlatma"),
        ("Moda & Giyim", "Dış Giyim"), ("Moda & Giyim", "Ayakkabı & Çanta"), ("Moda & Giyim", "Spor Giyim"),
        ("Süpermarket & Gıda", "Temel Gıda"), ("Süpermarket & Gıda", "Kahve & İçecek"), ("Süpermarket & Gıda", "Atıştırmalık"),
        ("Kozmetik & Bakım", "Cilt Bakımı"), ("Kozmetik & Bakım", "Parfüm & Deodorant"),
        ("Otomotiv & Yapı", "Oto Aksesuar"), ("Otomotiv & Yapı", "El Aletleri & Hırdavat"),
        ("Anne & Bebek", "Bebek Bezi & Bakım"), ("Kitap & Kırtasiye", "Akademik Yayınlar")
    ]
    ...
    kat_base_price = np.array([
        28500.0, 42000.0, 9500.0,
        18500.0, 4800.0, 1650.0,
        2400.0, 1950.0, 1450.0,
        650.0, 420.0, 180.0,
        980.0, 1650.0,
        2200.0, 3100.0,
        890.0, 540.0
    ...
    odeme_yontemleri = np.array([
        "Kredi Kartı (Tek Çekim)", "Taksitli Kredi Kartı", "Havale / EFT",
        "Dijital Cüzdan", "Kapıda Ödeme", "Kurumsal Açık Hesap"
    ], dtype=object)

    musteri_segmentleri = np.array([
        "Bireysel Standart", "Bireysel Plus / VIP", "KOBİ Ticari",
        "Kurumsal Enterprise", "Kamu / Akademik"
    ], dtype=object)

    satis_kanallari = np.array([
        "Mobil Uygulama (iOS/Android)", "Web Mağaza (Masaüstü)",
        "Pazaryeri Entegrasyonu", "Kurumsal B2B Portal", "Fiziksel Mağaza (POS)"
    ], dtype=object)

    kargo_firmalari = np.array([
        "Yurtiçi Lojistik", "Aras Ekspres", "MNG Kargo", "HepsiJet", "Trendyol Express", "PTT Kargo"
    ], dtype=object)

    # Pre-build rich dirty string pools (200,000 distinct dirty strings each) for fast vectorized sampling
    pool_size = 200_000
    null_tokens = ["Yok", "Bilinmiyor", "Belirtilmemiş", "N/A", "Boş", "-", "NULL", "Hatalı"]
    ...
    for chunk_idx in range(num_chunks):
        ...
        ts_ms = base_ts + islem_id * 1250 + rng.integers(0, 999, size=chunk_size, dtype=np.int64)
        ...
        col_odeme = odeme_yontemleri[rng.integers(0, len(odeme_yontemleri), size=chunk_size)]
        seg_idx = rng.integers(0, len(musteri_segmentleri), size=chunk_size)
        col_segment = musteri_segmentleri[seg_idx]
        col_kanal = satis_kanallari[rng.integers(0, len(satis_kanallari), size=chunk_size)]
        col_kargo = kargo_firmalari[rng.integers(0, len(kargo_firmalari), size=chunk_size)]
        ...
        satis_tutari = (base_p * rng.lognormal(mean=0.0, sigma=0.35, size=chunk_size)).astype(np.float32)
        ...
        birim_maliyet = (satis_tutari * rng.uniform(0.52, 0.78, size=chunk_size) + rng.normal(0, 120, size=chunk_size)).astype(np.float32)
        ...
        musteri_yasi[rng.random(chunk_size) < 0.002] = rng.choice(np.array([-5.0, 135.0, 199.0], dtype=np.float32), size=1)[0]
        musteri_yasi[rng.random(chunk_size) < 0.015] = np.nan

        # 16. Memnuniyet_Puani (1.00 - 5.00 correlated with segment)
        memnuniyet = np.clip(rng.normal(3.85 + 0.12 * seg_idx, 0.65, size=chunk_size), 1.0, 5.0).astype(np.float32)
        memnuniyet[rng.random(chunk_size) < 0.02] = np.nan

        # 17. Oturum_Suresi_Sn (Continuous float32)
        oturum_suresi = rng.exponential(scale=240.0, size=chunk_size).astype(np.float32) + np.float32(15.0)
        ...
        col_kirli_agirlik = dirty_agirlik_pool[rng.integers(0, pool_size, size=chunk_size)]
        col_kirli_indirim = dirty_indirim_pool[rng.integers(0, pool_size, size=chunk_size)]
        ...
        table = pa.table({
            ...
            "Kirli_Paket_Agirligi_Kg": pa.array(col_kirli_agirlik).dictionary_encode(),
            "Kirli_Indirim_Orani": pa.array(col_kirli_indirim).dictionary_encode(),
            "Kirli_Musteri_Yillik_Ciro": pa.array(col_kirli_ciro).dictionary_encode(),
        ...
        del table, islem_id, islem_zamani, satis_tutari, birim_maliyet, kargo_ucreti, net_kar
        if (chunk_idx + 1) % 5 == 0 or chunk_idx == 0:
            elapsed = time.time() - t0
            rows_done = (chunk_idx + 1) * chunk_size
            print(f"[{chunk_idx + 1:02d}/{num_chunks}] Wrote {rows_done:,} rows ({elapsed:.1f}s elapsed)")
            gc.collect()
    ...
    print(f"DONE! Generated {total_rows:,} rows x 22 columns in {total_time:.1f}s -> {out_path} ({size_gb:.2f} GB)")
