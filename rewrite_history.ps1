$git = "C:\Users\umutc\AppData\Local\GitHubDesktop\app-3.6.5\resources\app\git\cmd\git.exe"
...
# Start by checking out the very first commit as a detached HEAD
& $git checkout $hashes[0]
& $git commit --amend -m $mapping[$hashes[0]] --allow-empty

# Now loop through the rest, cherry-picking and amending
for ($i = 1; $i -lt $hashes.Length; $i++) {
    $h = $hashes[$i]
    Write-Host "Processing $h ..."
    & $git cherry-pick $h
    
    # If cherry pick somehow fails or has conflicts, skip it (shouldn't happen on a clean replay)
    if ($LASTEXITCODE -ne 0) {
        Write-Host "WARNING: Cherry-pick failed on $h! Skipping."
        & $git cherry-pick --abort
        continue
    }

    # Amend the message
    & $git commit --amend -m $mapping[$h] --allow-empty
}

# At the end, force the main branch to this new history
