...
def run_tests():
    ...
    d = j(res)
    assert_test("Sistem Sağlık Kontrolü (GET /health)", res.status_code == 200 and d.get('status') == 'ok')

    # 2. Load Sample Dataset
    res = client.post('/load_sample')
    d = j(res)
    assert_test("Örnek Veri Seti Yükleme (POST /load_sample)", res.status_code == 200 and d.get('success') is True,
                f"Satır: {d.get('total_rows')}, Sütun: {d.get('total_cols')}")

    # 3. Check Health
    res = client.get('/check_health')
    d = j(res)
    assert_test("Veri Kalite Kontrolü (GET /check_health)", res.status_code == 200 and 'has_issues' in d)
    ...
    assert_test("Kategorik Dilimleyici POST Sorgusu (POST /get_column_unique_values)",
                res.status_code == 200 and d.get('type') == 'cat' and len(d.get('categories', [])) > 0)
    ...
    assert_test("Canlı Regresyon ve Korelasyon Eğrisi (POST /get_regression_curve)",
                res.status_code == 200 and d.get('success') is True and 'regression' in d and 'correlation' in d)
