...
(function () {
  ...
  function bindRegEvents() {
    ...
    document
      .getElementById("regToggleCiBand")
      ?.addEventListener("change", () => {
        if (regCurrentData) renderRegressionScatterPlot(regCurrentData);
      });

    document
      .getElementById("btnPinRegChart")
      ?.addEventListener("click", pinRegressionToDashboard);

    document
      .getElementById("btnDownloadRegPng")
      ?.addEventListener("click", () => {
  ...
  function updateRegDiagnosticBadges(data) {
    ...
    const methodSymbol =
      data.corr_method === "spearman"
        ? "ρ"
        : data.corr_method === "kendall"
          ? "τ"
          : "r";
    ...
    if (valCorr) {
      valCorr.textContent =
        corr.coef != null ? `${methodSymbol} = ${corr.coef.toFixed(4)}` : "-";
    }
    ...
    if (valP) {
      ...
      if (p != null) {
        ...
        if (valPBadge)
          valPBadge.textContent =
            p < 0.05 ? "p < 0.05 (Güvenilir)" : "p ≥ 0.05 (Anlamsız)";
      } else {
        valP.textContent = "-";
  ...
  function renderCorrelationHeatmap(data) {
    ...
    if (
      ...
      if (container) {
        container.innerHTML =
          '<div style="display:flex; height:100%; align-items:center; justify-content:center; color:var(--muted); font-size:0.8rem;">En az 2 sayısal sütun gereklidir.</div>';
      }
      return;
    }
    ...
    const textValues = zValues.map((row) =>
      row.map((v) => (v != null ? v.toFixed(2) : "0.00")),
    );
    ...
    const trace = {
      ...
      hovertemplate:
        "<b>%{y}</b> ↔ <b>%{x}</b><br>Korelasyon: <b>%{z:.4f}</b><extra></extra>",
    };
  ...
  function pinRegressionToDashboard() {
    ...
    const clonedLayout =
      plotDiv && plotDiv.layout
        ? JSON.parse(JSON.stringify(plotDiv.layout))
        : null;
    ...
    const regPayload = {
      ...
      plotlyTraces: clonedTraces,
      plotlyLayout: clonedLayout,
      data: {
    ...
    if (typeof window.addChartToDashboard === "function") {
      ...
      dashboardCharts.push({
        ...
        plotlyTraces: clonedTraces,
        plotlyLayout: clonedLayout,
        regData: regPayload.data,
        chartData: {
          x: regCurrentData.scatter_x || [],
          y: regCurrentData.scatter_y || [],
          x_name: regCurrentData.x_col,
          y_name: regCurrentData.y_col,
        },
      });
  ...
  function openRegressionStudioWithVars(xCol, yCol, modelType = "linear") {
    if (typeof window.goToStep3 === "function") {
      window.goToStep3(window.currentPlotType || "scatter");
    }
