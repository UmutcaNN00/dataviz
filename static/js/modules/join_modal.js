...
async function executeMerge() {
  ...
  fd.append('key1', mergeKey1Select ? mergeKey1Select.value : '');
  fd.append('key2', mergeKey2Select ? mergeKey2Select.value : '');
...
function initJoinModalListeners() {
  ...
  ['btnOpenMergeModal', 'btnOpenMergeModalS2', 'btnOpenMergeModalS3'].forEach(id => {
    document.getElementById(id)?.addEventListener('click', openDataMergeModal);
  });

  document.getElementById('btnCloseMergeModal')?.addEventListener('click', () => {
    dataMergeModal?.classList.add('hidden');
  });
