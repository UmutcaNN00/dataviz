...
def get_sheet_preview():
    ...
    if excel_file:
        if sheet_name and sheet_name in sheet_names:
            target_df = excel_file.get(sheet_name) if isinstance(excel_file, dict) else excel_file.parse(sheet_name)
        elif sheet_names:
            resolved_sheet_name = sheet_names[0]
            target_df = excel_file.get(resolved_sheet_name) if isinstance(excel_file, dict) else excel_file.parse(resolved_sheet_name)
    ...
    preview_df = target_df.head(limit).replace({np.nan: None, pd.NA: None})
