...
def get_stats():
    ...
    if not cols or active_df.empty:
        return jsonify(
            {"stats": {}, "advanced": {}, "total_active_rows": len(active_df)}
        )

    try:
        proj_cols = list(
            dict.fromkeys(
                [c for c in cols if c in active_df.columns]
                + ([x_col] if x_col and x_col in active_df.columns else [])
            )
        )
        sub_df = active_df[proj_cols].replace([np.inf, -np.inf], np.nan)
        stats = compute_column_statistics(sub_df, cols)
        advanced = compute_advanced_stats(
            sub_df, cols, x_col=x_col, corr_method=corr_method, reg_model=reg_model
        )
...
def generate_insight():
    ...
    try:
        result = generate_academic_insight(
            stats, advanced=advanced, chart_type=chart, x_col=x_col, y_cols=y_cols
        )
...
def get_regression_studio_data_route():
    ...
    try:
        ...
        filters = data.get("filters", [])
        try:
            max_scatter = max(100, min(int(data.get("max_scatter", 2500)), 10000))
        ...
        if not x_col or not y_col:
            if len(num_cols) >= 2:
                ...
                return jsonify(
                    {"error": "Regresyon için en az 2 sayısal değişken gereklidir."}
                ), 400
        ...
        if total_valid > max_scatter:
            sample_indices = np.random.RandomState(42).choice(
                total_valid, size=max_scatter, replace=False
            )
        ...
        resolved_p = (
            reg_result.get("p_value")
            if reg_result.get("p_value") is not None
            else corr_result.get("p_value")
        )
        stats_mock = {
            y_col: {
                ...
                "std": float(np.std(y_clean, ddof=1)) if total_valid > 1 else 0.0,
                "count": total_valid,
            }
        }
        adv_mock = {
        ...
        try:
            insight = generate_academic_insight(
                stats_mock,
                advanced=adv_mock,
                chart_type="Scatter & Regresyon",
                x_col=x_col,
                y_cols=[y_col],
            )
